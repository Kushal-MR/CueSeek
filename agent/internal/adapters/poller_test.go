...
func TestCacheStatusSinceTracksTransitions(t *testing.T) {
	...
	c.Put("jellyfin", domain.Health{Status: domain.StatusHealthy, ObservedAt: now})
	...
	now = now.Add(time.Minute)
	c.Put("jellyfin", domain.Health{Status: domain.StatusUnreachable, ObservedAt: now})
...
func TestCacheSnapshots(t *testing.T) {
	...
	c.Put("a", domain.Health{Status: domain.StatusHealthy, ObservedAt: time.Now().UTC()})
...
func TestCacheIsConcurrencySafe(t *testing.T) {
	...
	for range 8 {
		...
		go func() {
			...
			for i := range 100 {
				id := []string{"a", "b", "c"}[i%3]
				c.Put(id, domain.Health{Status: domain.StatusHealthy, ObservedAt: time.Now().UTC()})
				c.Get(id)
				c.Snapshots()
			}
		}()
