...
— monitoring and control. Its construction, rules, and the seven rounds it took are in
**`DESIGN.md` §13**, not repeated here.
...
| Legacy rasters | **10 `.webp` files deleted** — both apps have `minSdk` ≥ 26, so every device they install on uses the adaptive icon, and the rasters were dead weight carrying the old mark |
| Name | `CueSeek` in both launchers, unchanged |
| Splash | the system's, built from the adaptive icon — no code |

**Every icon file is generated by `docs/brand/icon.py`** — seven Android drawables and two SVG
masters, from one geometry. None is edited by hand, because three copies of a mark stay
identical only if none of them is maintained separately. The script refuses to emit `--`
inside an XML comment, after two hand-written comments failed the build that way.

**"A Wear launcher icon is not the phone icon scaled down."** The watch uses the same mark,
deliberately — one identity — but it was designed *for* the round crop from the first round
rather than adapted to it afterwards. Every candidate was previewed in a circular mask and at
24dp before any was chosen, and every point of the final mark lies inside the 66dp safe
circle.
