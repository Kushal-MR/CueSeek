...
The mark is called **Swell**. It is a dial — a 270° track open at the bottom — which is the
one image that means both halves of what CueSeek does: a dial is something you **read** and
something you **turn**. The active arc swells from a hairline at 7:30 into a round handle at
1:30; after a gap, a tail tapers back to a hairline at 4:30. Nothing in it steps: width
changes continuously, eased at both ends.

<img src="brand/cueseek-icon.svg" width="96" alt="The CueSeek icon: a green tile carrying a light green dial that swells into a round handle at half past one.">

### Two renderings of one shape

| | where | why |
| --- | --- | --- |
| **Graded** | launcher, Play Store, website | a sweep gradient, brightest at the handle, so tone follows weight |
...
On the 108 × 108 adaptive-icon canvas (72 visible, 66 guaranteed), every point lies within
r33 of the centre, so the mark survives both the phone's squircle and the watch's circle.

| | |
| --- | --- |
| Track | radius 24, centred at **(55, 57.5)** — nudged right and down because an arc open at the bottom with a handle at top-right carries its weight up and right |
| Width | 1.6 at both ends, 7.0 at the handle, eased by smoothstep so the swell has no visible kink |
| Handle | r 7.5 at 315° (1:30) |
| Gaps | **32.67°** either side of the handle: its radius, 2.5 of clearance, and the cap of a 7-wide end — computed, not eyeballed |

**Every icon file is generated by [`brand/icon.py`](brand/icon.py)** — both apps' foreground,
monochrome and background drawables, the watch's tile preview and complication icon, and the
two SVG masters. The phone, the watch and these docs each carry a copy of the mark, and the
only thing keeping them identical is that none of them is edited by hand. Change the script,
run it, commit the output.

The tapers are filled outlines, not strokes. A stroke has one width, and the whole point of
the mark is that its width does not.

### Rules

- **Do not edit a generated file.** Each says so in its header.
- **Do not add to it.** Every attempt to make the mark say "monitoring" more literally — a
  heartbeat, a radar sweep, a needle, a scope — made it less minimal, and was rejected for
  exactly that. The restraint is the design.
- **No monogram.** A "CS" says who the app is, not what it does, and a letter nested in a C
  drifts toward ©. Decided, not deferred.
- **No colours outside §3.** The tile and both grade stops are palette tokens.

### How it was chosen

Seven rounds, recorded so nobody has to reconstruct them. A gauge, a pulse and a monogram
first; then dots-as-status (read as a die, or an apps grid) and rings-with-a-marker; then the
ring family alone, refined toward "monitoring and control", which produced the dial with a
handle. The first dial dropped from thick to thin at the handle — a step the eye read as a
break — and the swell replaced the step. Two further rounds tried to make the mark more
literal and lost the minimalism each time.

**Rejected on principle: AI image generation.** A generator returns a raster, and a launcher
icon is a VectorDrawable with a monochrome layer — neither of which a PNG can be.

### Still open

**Whether the handle earns its place.** It was framed as the thing you would turn — the
*control* half of the product — and on first sight that is not self-evident; it was asked
